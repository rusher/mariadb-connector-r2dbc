...
public class IntParameterTest extends BaseTest {
  @BeforeAll
  ...
  public void beforeEach() {
    sharedConn.createStatement("TRUNCATE TABLE IntParam").execute().blockLast();
  }

  @Test
  void nullValue() {
    sharedConn
        .createStatement("INSERT INTO IntParam VALUES (?,?,?)")
  ...
  void bigIntValue() {
    sharedConn
        ...
        .bind(1, new BigInteger("922337203"))
        .bind(2, new BigInteger("-9"))
        .execute()
        .blockLast();
    validate(Optional.of(1), Optional.of(922337203), Optional.of(-9));
  }
  ...
  void decimalValue() {
    sharedConn
        ...
        .bind(1, new BigDecimal("922337203"))
  ...
  private void validate(Optional<Integer> t1, Optional<Integer> t2, Optional<Integer> t3) {
    sharedConn
        .createStatement("SELECT * FROM IntParam")
        .execute()
        .flatMap(
            r ->
                r.map(
